...
void vgit::Delta::handle_run(std::streampos offset, uint8_t byte, bool running) {
...
vgit::Delta::Delta(const fs::path& __old, const fs::path& __new) : _fsize(fs::file_size(__new)) {
...
void vgit::Delta::apply(const fs::path& __destination) {
    ...
    std::fstream dststream(__destination, std::ios::binary | std::ios::in | std::ios::out);
    for (const auto& run : _runs) {
        dststream.seekp(run.offset);
        dststream.write(reinterpret_cast<const char*>(run.diff.data()), run.diff.size());
    }
}
...
void vgit::Delta::serialize(const fs::path& __destination) {
    ...
    for (const auto& run : _runs) {
        out.write(reinterpret_cast<const char*>(&run.offset), sizeof(run.offset));
